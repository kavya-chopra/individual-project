...
def plain_old_data_to_json_object(obj):
    ...
    #     return [sydx.__serialise(x) for x in obj]
        ...
        return {
            'value_type': 'tuple',
            'value': [sydx.__serialise(x) for x in list(obj)]
            # 'value': list(obj)
        }
    elif isinstance(obj, dict):
        return {
            'value_type': 'dict',
            'value': {x: sydx.__serialise(y) for x, y in obj.items()}
        }
    elif obj is None:
        return None
    else:
    ...
    if isinstance(json_object, dict):
        if 'value_type' in json_object:
            if json_object['value_type'] == 'int':
                ...
                return [sydx.__deserialise(x) for x in json_object['value']]
            elif json_object['value_type'] == 'tuple':
                return tuple([sydx.__deserialise(x) for x in json_object['value']])
            elif json_object['value_type'] == 'dict':
                return {x: sydx.__deserialise(y) for x, y in json_object['value'].items()}
        else:
